...
覆盖 A 股全市场（沪/深/北）**27 张表**，行情、估值、资金、信号、技术指标、财务、外围七层持久化数据，从 2010 年至今累计约 **1500 万行**。同时封装了研报、新闻、公告、快讯等在线数据接口，供应用层按需即时获取。
...
本项目解决了这些问题。它把 6 个数据源的原始接口统一封装成 27 张关系表，每天自动增量更新，开箱即用。你可以直接在这套数据之上构建：
...
| 表 | 内容 | 业务价值 |
|----|------|---------|
| `indicator_values` | **30 项技术指标**（MACD/KDJ/RSI/BOLL/BIAS/PSY/TRIX/DPO/MTM/ROC/EXPMA/BBI/DFMA/DMI/ATR/WR/CCI/CR/KTN/XSII/OBV/VR/EMV/MASS/MFI/BRAR/ASI/ZHUOYAO/BIAS_SIGNAL/TAQ），每日按 D/W/M 三种频率计算 | 金叉死叉/超买超卖/布林突破等信号扫描，无需每次重算 |

> **分析场景：** RSI<30 超卖筛选；MACD 金叉买入信号；BOLL 突破上轨追涨。
...
| 数据源 | 协议 | 覆盖表 | 特性 | 替代方案 |
|--------|------|--------|------|---------|
| **easy_tdx**（通达信协议封装） | TCP | 6 张核心表（stock_universe, classification, concept_blocks, daily_ohlcv, capital_flow, global_markets）+ board_daily + 实时行情 + indicator_values 计算依赖 | 盘中实时，数据最全，含北交所；TCP 有限流，回补并发 ≤ 3 | baostock（缺北交所）、akshare |
...
| **baostock** | TCP | trade_calendar, daily_valuation（历史） | 免费稳定，但仅沪深，无北交所 | easy_tdx |
...
```
6 个数据源 ──→ 20+ 个 fetcher ──→ 3 个 Wave ──→ DuckDB（27 张表）
                      (并发调度)         (分层执行)
```
